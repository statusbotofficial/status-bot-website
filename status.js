        ...
        async function fetchBotStats() {
            try {
                const response = await fetch('https://status-bot-backend.onrender.com/api/bot-stats');
                if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`);
                
                const data = await response.json();
                
                document.getElementById('serverCount').textContent = data.servers || 0;
                document.getElementById('pingLatency').textContent = data.ping ? `${data.ping}ms` : "N/A";
            } catch (error) {
                console.error('Error fetching bot stats:', error);
                document.getElementById('serverCount').textContent = "Offline";
                document.getElementById('pingLatency').textContent = "Offline";
            }
        }

        // Refresh bot stats every 30 seconds
